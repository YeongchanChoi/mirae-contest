...
def process_stock_summary(stock_name, date_range_str):
    ...
    if os.path.exists(summary_file_path):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(analyze_sentiment, row['요약내용']): row['날짜']
                for index, row in df.iterrows()
            }

            for future in as_completed(futures):
                pubDate_str = futures[future]
                try:
                    confidence_sum = future.result()
                    sentiment_dict[pubDate_str].append(confidence_sum)
                    print(f"Sentiment confidence (positive - negative) for {pubDate_str}: {confidence_sum}")
                except Exception as e:
                    print(f"Error analyzing sentiment for date {pubDate_str}: {e}")
